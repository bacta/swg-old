...
public final class DataTableWriter {
    ...
    private static String unquotify(String s) {
    ...
    public static boolean isXmlFile(final String fileName)  {
        Preconditions.checkNotNull(fileName);
        return fileName.toLowerCase().equalsIgnoreCase("xml");
    }
    ...
    {
        columnType.mangleValue(unquotify(value));

        switch (columnType.getBasicType()) {
            case Int:
                return new DataTableCell(value.isEmpty() ? 0 : Integer.parseInt(value));
            case Float:
                return new DataTableCell(value.isEmpty() ? 0 : Float.parseFloat(value));
            case String:
            case Comment:
